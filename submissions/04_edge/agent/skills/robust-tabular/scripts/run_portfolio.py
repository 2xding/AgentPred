...
def folds_for(y: np.ndarray, n_rows: int, seed: int = SEED) -> StratifiedKFold:
...
def seeds_for(n_rows: int) -> tuple[int, ...]:
    """Bag more splits on small data, where fold noise dominates; stay cheap on large."""
    if n_rows <= 20000:
        return SEEDS
    return (SEED,)


def averaged(runner, y, splitters):
    """Run a CV routine once per split and average OOF and test predictions."""
    oof_total = None
    test_total = None
    for splitter in splitters:
        oof, test_predictions = runner(splitter)
        oof_total = oof if oof_total is None else oof_total + oof
        test_total = test_predictions if test_total is None else test_total + test_predictions
    n = len(splitters)
    return oof_total / n, test_total / n
...
def main() -> None:
    ...
    splitters = [folds_for(y, len(train), seed) for seed in seeds_for(len(train))]
    models: dict[str, dict] = {}
    errors: dict[str, str] = {}

    runners = [
        ("catboost", lambda: averaged(lambda sp: cv_catboost(native_train, y, native_test, cat_cols, sp), y, splitters)),
        ("lightgbm", lambda: averaged(lambda sp: cv_lightgbm(encoded_train, y, encoded_test, encoded_cat_cols, sp), y, splitters)),
        ("extra_trees", lambda: averaged(lambda sp: cv_extra_trees(encoded_train, y, encoded_test, sp), y, splitters)),
        ("logistic", lambda: averaged(lambda sp: cv_logistic(train, y, test, features, sp), y, splitters)),
    ]
